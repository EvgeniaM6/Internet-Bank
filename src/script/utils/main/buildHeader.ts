...
class BuildHeader {
  anonimHeader() {
    ...
    <div class="header__down">
        <div class="header__login">${config.lang === 'en' ? 'Log In' : 'Войти'}</div>
  ...
  logHeader() {
    ...
    logout.textContent = config.lang === 'en' ? 'Log Out' : 'Выйти';
    ...
    account.textContent = config.lang === 'en' ? 'Account' : 'Аккаунт';
    account.id = 'account';
    ...
    cardCreator.textContent = config.lang === 'en' ? 'Card Creator' : 'Создать карточку';
    cardCreator.id = 'card';

    const stocks = document.createElement('li');
    stocks.classList.add('header__nav-item');
    stocks.classList.add('header__nav-stocks');
    stocks.textContent = config.lang === 'en' ? 'Stocks' : 'Акции';
    stocks.id = 'stocks';
  ...
  adminHeader() {
    ...
    admin.textContent = config.lang === 'en' ? 'Administration' : 'Админка';
    admin.id = 'admin';
